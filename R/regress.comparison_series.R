...
regress.comparison_series <- function(
    x,
    formula,
    model = "ols",
    start_year = NULL,
  ...
  if (!inherits(formula, "formula")) {
    stop(
      "`formula` må være en R-formel, for eksempel ",
  ...
  dependent_variable <- as.character(
    formula_response
  )
  ...
  if (length(independent_variables) == 0) {
    stop(
      "Formelen må inneholde minst én forklaringsvariabel.",
      call. = FALSE
    )
  }
  ...
  analysis_data <- tibble::as_tibble(
    ...
    dplyr::filter(
      .data$Serie_id %in% formula_variables
    )
  ...
  series_lookup <- tibble::tibble(
    Serie_id = analysis_data$Serie_id,
  ...
  if (nrow(inconsistent_display_names) > 0) {
    stop(
      "Minst én `Serie_id` har flere ulike display-navn.",
      call. = FALSE
    )
  }
  ...
  model_frame <- tryCatch(
    ...
    error = function(error) {
      stop(
        "Kunne ikke evaluere regresjonsformelen: ",
  ...
  number_of_candidate_observations <- nrow(
    model_frame
  )
  
  number_of_observations <- sum(
    complete_rows
  )
  ...
  if (number_of_observations == 0) {
    stop(
      "Ingen komplette observasjoner er tilgjengelige for modellen.",
      call. = FALSE
    )
  }
  ...
  dependent_display_name <- series_lookup |>
    dplyr::filter(
      .data$Serie_id == dependent_variable
    ) |>
    dplyr::pull(
      .data$Display_navn
    )
  ...
  independent_display_names <- series_lookup |>
    dplyr::filter(
      .data$Serie_id %in% independent_variables
    ) |>
    dplyr::select(
      Serie_id,
      Display_navn
  ...
  result <- list(
    ...
    number_of_incomplete_observations =
      number_of_incomplete_observations,
    estimated_start_year = min(
      estimated_years
    ),
    estimated_end_year = max(
      estimated_years
    ),
