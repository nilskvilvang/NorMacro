...
autocorrelate.comparison_series <- function(x,
  ...
  results <- lapply(series_ids, function(series_id) {
    series_data <- analysis_data |>
      dplyr::filter(.data$Serie_id == series_id) |>
      dplyr::select(Aar, Verdi) |>
      dplyr::arrange(.data$Aar)
    
    lag_results <- lapply(lags, function(lag_value) {
      current_data <- series_data |>
        dplyr::rename(Verdi = Verdi)
      
      lagged_data <- series_data |>
        dplyr::transmute(Aar = .data$Aar + lag_value,
                         Verdi_lag = .data$Verdi)
      
      pair_data <- dplyr::inner_join(current_data, lagged_data, by = "Aar") |>
        dplyr::arrange(.data$Aar)
      
      valid_pairs <- pair_data |>
        dplyr::filter(!is.na(.data$Verdi), !is.na(.data$Verdi_lag))
      
      number_of_observations <- nrow(valid_pairs)
      ...
      autocorrelation <- if (number_of_observations < 2) {
        NA_real_
      } else {
    ...
    dplyr::bind_rows(lag_results)
  })
  
  result <- dplyr::bind_rows(results) |>
    dplyr::left_join(series_lookup, by = "Serie_id") |>
    dplyr::select(
      Serie_id,
      Display_navn,
      Lag,
    ...
    dplyr::arrange(.data$Serie_id, .data$Lag)
  
  if (format) {
    result <- result |>
      dplyr::mutate(Autokorrelasjon = format_number(.data$Autokorrelasjon, digits = 3))
  }
  ...
  class(result) <- c("comparison_series_autocorrelation", class(result))
