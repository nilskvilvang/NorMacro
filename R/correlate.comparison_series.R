...
correlate.comparison_series <- function(
    x,
    method = c(
      "pearson",
      "spearman",
  ...
  if (
    ...
    stop(
      "`include_diagonal` må være TRUE eller FALSE.",
      call. = FALSE
    )
  }
  ...
  if (length(series_ids) < 2) {
    stop(
      "`correlate()` krever minst to serier.",
      call. = FALSE
    )
  }
  ...
  if (format) {
    result <- result |>
      dplyr::mutate(
        Korrelasjon = format_number(
          .data$Korrelasjon,
          digits = 3
        ),
        P_verdi = format_pvalue(
          .data$P_verdi
        )
      )
  }
