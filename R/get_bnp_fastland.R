...
get_bnp_fastland <- function(refresh = FALSE) {
  cache_get(
    ...
    refresh = refresh,
    fun = function() {
      query <- list(Makrost = "bnpb.nr23_9fn",
                    ContentsCode = "Faste",
                    Tid = "*")
      
      bnp <- ssb_get(url = "https://data.ssb.no/api/v0/no/table/nk/nk03/knr/SBMENU5140/NRMakroHov", query = query)
      ...
      bnp |>
        ...
        dplyr::arrange(Aar) |>
        dplyr::mutate(BNP_Fastland_vekst =
