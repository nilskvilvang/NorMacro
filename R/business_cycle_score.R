...
business_cycle_score <- function(data = NULL,
                                 bnp_weight = 2,
  ...
  if (include_yield_curve) {
    result <- result |>
      dplyr::mutate(
        Score_rentekurve = dplyr::case_when(Rentekurve < 0 ~ -1, Rentekurve >= 0 ~ 1, TRUE ~ NA_real_) * yield_curve_weight
      )
  } else {
    result <- result |>
      dplyr::mutate(Score_rentekurve = NA_real_)
  }
  
  score_cols <- c("Score_BNP",
                  "Score_ledighet",
                  "Score_konjunktur",
                  "Score_kapasitet")
  ...
  result |>
    dplyr::mutate(
      Score = rowSums(dplyr::pick(dplyr::all_of(score_cols)), na.rm = TRUE),
      Antall_indikatorer = rowSums(!is.na(dplyr::pick(
