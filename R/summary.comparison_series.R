...
summary.comparison_series <- function(object, ...) {
  ...
  result <- data |>
    dplyr::group_by(.data$Serie_id, .data$Display_navn) |>
    dplyr::summarise(
      Foerste_aar = as.integer(min(.data$Aar, na.rm = TRUE)),
      Siste_aar = as.integer(max(.data$Aar, na.rm = TRUE)),
      Foerste_gyldige_aar = {
        valid_years <- .data$Aar[!is.na(.data$Verdi)]
        
        if (length(valid_years) == 0) {
          NA_integer_
        } else {
          as.integer(min(valid_years))
        }
      },
      Siste_gyldige_aar = {
        valid_years <- .data$Aar[!is.na(.data$Verdi)]
        ...
        if (length(valid_years) == 0) {
          ...
          as.integer(max(valid_years))
        }
      },
      Observasjoner = sum(!is.na(.data$Verdi)),
      Manglende = sum(is.na(.data$Verdi)),
      Gjennomsnitt = if (all(is.na(.data$Verdi))) {
        NA_real_
      } else {
        mean(.data$Verdi, na.rm = TRUE)
      },
      Median = if (all(is.na(.data$Verdi))) {
        NA_real_
      } else {
        stats::median(.data$Verdi, na.rm = TRUE)
      },
      ...
      Standardavvik = if (sum(!is.na(.data$Verdi)) < 2) {
        NA_real_
      } else {
        stats::sd(.data$Verdi, na.rm = TRUE)
      },
      .groups = "drop"
    )
  ...
  class(result) <- c("comparison_series_summary", class(result))
