...
combine_series <- function(norway = NULL,
  ...
  if (!is.null(norway)) {
    ...
    norway_result <- norway_data |>
      dplyr::select(Aar, dplyr::all_of(norway)) |>
      tidyr::pivot_longer(cols = -Aar,
                          names_to = "Variabel",
                          values_to = "Verdi") |>
      dplyr::left_join(norway_metadata |>
                         dplyr::select(Variabel, Display_navn, Enhet, Kilde),
                       by = "Variabel") |>
      dplyr::mutate(
        Serie_id = paste0("NO_", .data$Variabel),
        Datasett = "Norge",
        Land = "NO",
        .before = "Variabel"
      )
  ...
  if (!is.null(international)) {
    ...
    available_countries <- unique(international_data$Land)
    
    requested_countries <- names(international)
    
    missing_countries <- setdiff(requested_countries, available_countries)
    ...
    for (country in requested_countries) {
      ...
      missing_variables <- setdiff(variables, names(international_data))
      ...
      country_result <- international_data |>
        ...
        dplyr::mutate(
          Serie_id = paste0(country, "_", .data$Variabel),
    ...
    results[["international"]] <- dplyr::bind_rows(international_results)
  }
