...
compare_periods <- function(variables, start_year, end_year, data = NULL) {
  if (is.null(data)) {
  ...
  result <- purrr::map_dfr(variables, function(variable) {
    ...
    start_value <- if (length(start_value) == 0)
      NA_real_
    else
      start_value
    end_value   <- if (length(end_value) == 0)
      NA_real_
    else
      end_value
  ...
  result |>
    dplyr::left_join(metadata |>
                     ...
                     by = "Variabel") |>
    dplyr::select(Variabel, Kategori, Beskrivelse, Enhet, dplyr::everything())
}
