...
get_valutakurs <- function(refresh = FALSE) {
  cache_get(
    ...
    fun = function() {
      ...
      valuta_raw <- rio::import(url_valuta, format = "csv")
      
      valuta_raw |>
        dplyr::transmute(
          Aar = as.integer(TIME_PERIOD),
          Valutakurs_I44 = readr::parse_number(OBS_VALUE, locale = readr::locale(decimal_mark = ","))
        ) |>
        dplyr::arrange(Aar) |>
        dplyr::mutate(Valutakurs_I44_vekst =
