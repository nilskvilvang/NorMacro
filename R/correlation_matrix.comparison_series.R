...
correlation_matrix.comparison_series <- function(
    variables,
    start_year = NULL,
    end_year = NULL,
    use = c(
  ...
  number_of_series <- dplyr::n_distinct(
    analysis_data$Serie_id
  )
  
  if (number_of_series < 2L) {
    stop(
      "`correlation_matrix()` krever minst to serier.",
      call. = FALSE
    )
  }
  ...
  class(result) <- c(
    "comparison_series_correlation_matrix",
    class(result)
  )
