...
variable_summary <- function(variable,
  ...
  growth <- growth_table(variables = variable,
                         data = data,
                         periods = c(1, 5, 10))
  ...
  correlation_variables <- intersect(correlation_variables, names(data))
  ...
  if (length(correlation_variables) > 0) {
    ...
    corr <- correlation_matrix(variables = corr_vars, data = data)
    
    correlations <- tibble::tibble(Variabel = rownames(corr),
