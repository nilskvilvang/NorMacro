...
get_offentlig_finans <- function(refresh = FALSE) {
  cache_get(
    name = "offentlig_finans",
    refresh = refresh,
    fun = function() {
      ...
      offentlig_raw |>
        dplyr::mutate(Aar = as.integer(ar),
                      beholdninger = as.numeric(beholdninger)) |>
        dplyr::select(Aar, finansobjekt, beholdninger) |>
        tidyr::pivot_wider(names_from = finansobjekt, values_from = beholdninger) |>
        dplyr::rename(Offentlig_gjeld = `GJELD I ALT`,
